"""
Извлечение признаков с окном 30 секунд и 15-секундной агрегацией (13 признаков).
...
Агрегаты по 15-секундным подокнам (6):
- speed_15s_mean/max/std — статистики скорости
- angle_15s_mean/std — статистики углов
- jump_15s_max — максимальная частота прыжков
...
from src.feature_extractor import extract_features_30s_15s
...
def process_file(
    ...
    for tid, group in df.groupby('track_id'):
        ...
        feats = extract_features_30s_15s(group, fps=fps, overlap=overlap)
...
def main():
    parser = argparse.ArgumentParser(
        description='Извлечение 13 признаков из 30-секундных окон с 15-сек агрегатами'
    )
    ...
    # Общие параметры
    parser.add_argument(
        '--output', type=str, default='dataset_30s15s_robust.csv',
        help='Путь к выходному CSV файлу (по умолчанию: dataset_30s15s_robust.csv)'
    )
    parser.add_argument(
    ...
    if args.config:
        ...
        print("  python scripts/extract_features_30s_15s.py --config config.json --output dataset.csv")
        print("  python scripts/extract_features_30s_15s.py \\")
    ...
    print(f"\n🔍 Извлечение признаков (окно 30 сек, подокно 15 сек, 13 признаков)...")
    ...
    print(f"📐 Окно: 30 секунд ({30 * args.fps} кадров)")
    print(f"📐 Подокно: 15 секунд ({15 * args.fps} кадров)")
    print(f"🔥 Перекрытие окон: {args.overlap*100:.0f}%")
    print(f"\n📋 Признаки (13 шт.):")
