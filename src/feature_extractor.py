...
def process_file(
    ...
    fps: int = 30, 
    sub_type: str = '30s',
    overlap: float = 0.75,  # 🔥 НОВЫЙ ПАРАМЕТР
    start_frame: int = 0    # 🔥 НОВЫЙ ПАРАМЕТР: фильтрация по времени
) -> pd.DataFrame:
    """
    ...
    if sub_type == '1min':
        extract_func = lambda g: extract_features_5min_1min(g, fps=fps, overlap=overlap)
        sub_desc = "1 минута"
    else:
        extract_func = lambda g: extract_features_5min_30s(g, fps=fps, overlap=overlap)
        sub_desc = "30 секунд"
